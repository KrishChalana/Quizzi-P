...
function Navbar() {
  ...
  function onClickButtons(e){
      if (e.target.id){
        const redirecttarget = "/"+e.target.id
        return naviagate(redirecttarget)
      }
   
  }
  return (
    <>
      <div class="fixed bottom-40 left-100% flex flex-col justify-center items-center  transition-all duration-[450ms] ease-in-out w-16 mx-2">
        <article class="border border-solid border-gray-700 w-full ease-in-out duration-500 left-0 rounded-2xl inline-block shadow-lg shadow-black/15 ">
          <label
            for="dashboard"
